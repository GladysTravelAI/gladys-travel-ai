...
export default function Navbar() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('gladys-agent-context');
      if (saved) {
        const ctx = JSON.parse(saved);
        setNotifications(ctx.watchlist?.filter((w: any) => w.alertEnabled).length || 0);
      }
    } catch {}
  }, []);
              ...
              <div className="relative">
                ...
                >
                  {userProfile?.profileImage ? (
                    ...
                    <div className="w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-white font-bold text-sm shadow"
