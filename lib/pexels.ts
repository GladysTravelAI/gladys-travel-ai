...
export default async function fetchPexelsImages(destination: string): Promise<string[]> {
  ...
  if (landmarks && landmarks.length > 0) {
    // Fetch images for top 3 landmarks
    const landmarkPromises = landmarks.slice(0, 3).map(async (landmark) => {
      try {
        const query = encodeURIComponent(`${landmark} ${destination}`);
        ...
        if (!response.ok) {
          console.error(`Pexels error for ${landmark}:`, response.status);
          return [];
        }
  ...
  if (allImages.length < 3) {
    try {
      const query = encodeURIComponent(`${destination} landmark skyline iconic`);
      const response = await fetch(
      ...
      console.error("Pexels fallback search error:", error);
    }
  }
