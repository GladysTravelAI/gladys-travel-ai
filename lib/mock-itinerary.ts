...
export interface EventAnchor {
  eventName:   string;
  eventType:   'sports' | 'music' | 'festival' | 'festivals' | 'other';
  eventDate:   string;
  eventDay:    number;
  venue:       string;
  city:        string;
  country?:    string;
  // Live match / setlist support
  fixtureId?:  string;   // API-Football fixture ID (football events)
  attraction?: string;   // Artist/band name (music events — used by setlist.fm)
}
