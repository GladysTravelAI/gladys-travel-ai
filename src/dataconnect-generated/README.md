...
- [**Queries**](#queries)
  ...
  - [*GetPrivateTripsForUser*](#getprivatetripsforuser)
- [**Mutations**](#mutations)
  - [*CreatePublicTrip*](#createpublictrip)
  - [*CreatePrivateTripForUser*](#createprivatetripforuser)
...
## GetPrivateTripsForUser
You can execute the `GetPrivateTripsForUser` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getPrivateTripsForUser(): QueryPromise<GetPrivateTripsForUserData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getPrivateTripsForUser(dc: DataConnect): QueryPromise<GetPrivateTripsForUserData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getPrivateTripsForUserRef:
```typescript
const name = getPrivateTripsForUserRef.operationName;
console.log(name);
```

### Variables
The `GetPrivateTripsForUser` query has no variables.
### Return Type
Recall that executing the `GetPrivateTripsForUser` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetPrivateTripsForUserData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetPrivateTripsForUser`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getPrivateTripsForUser } from '@dataconnect/generated';


// Call the `getPrivateTripsForUser()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getPrivateTripsForUser();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getPrivateTripsForUser(dataConnect);

console.log(data.trips);

// Or, you can use the `Promise` API.
getPrivateTripsForUser().then((response) => {
  const data = response.data;
  console.log(data.trips);
});
```

### Using `GetPrivateTripsForUser`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getPrivateTripsForUserRef } from '@dataconnect/generated';


// Call the `getPrivateTripsForUserRef()` function to get a reference to the query.
const ref = getPrivateTripsForUserRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getPrivateTripsForUserRef(dataConnect);
...
const { data } = await createPublicTrip({ name: ..., destination: ..., startDate: ..., endDate: ..., description: ..., });
...
// Variables can be defined inline as well.
const ref = createPublicTripRef({ name: ..., destination: ..., startDate: ..., endDate: ..., description: ..., });
...
## CreatePrivateTripForUser
You can execute the `CreatePrivateTripForUser` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
createPrivateTripForUser(vars: CreatePrivateTripForUserVariables): MutationPromise<CreatePrivateTripForUserData, CreatePrivateTripForUserVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
createPrivateTripForUser(dc: DataConnect, vars: CreatePrivateTripForUserVariables): MutationPromise<CreatePrivateTripForUserData, CreatePrivateTripForUserVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the createPrivateTripForUserRef:
```typescript
const name = createPrivateTripForUserRef.operationName;
console.log(name);
```

### Variables
The `CreatePrivateTripForUser` mutation requires an argument of type `CreatePrivateTripForUserVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `CreatePrivateTripForUser` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `CreatePrivateTripForUserData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface CreatePrivateTripForUserData {
  trip_insert: Trip_Key;
}
```
### Using `CreatePrivateTripForUser`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, createPrivateTripForUser, CreatePrivateTripForUserVariables } from '@dataconnect/generated';

// The `CreatePrivateTripForUser` mutation requires an argument of type `CreatePrivateTripForUserVariables`:
const createPrivateTripForUserVars: CreatePrivateTripForUserVariables = {
  name: ..., 
  destination: ..., 
  startDate: ..., 
  endDate: ..., 
...
// Call the `createPrivateTripForUser()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await createPrivateTripForUser(createPrivateTripForUserVars);
// Variables can be defined inline as well.
const { data } = await createPrivateTripForUser({ name: ..., destination: ..., startDate: ..., endDate: ..., description: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await createPrivateTripForUser(dataConnect, createPrivateTripForUserVars);

console.log(data.trip_insert);

// Or, you can use the `Promise` API.
createPrivateTripForUser(createPrivateTripForUserVars).then((response) => {
  const data = response.data;
  console.log(data.trip_insert);
});
```

### Using `CreatePrivateTripForUser`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, createPrivateTripForUserRef, CreatePrivateTripForUserVariables } from '@dataconnect/generated';
...
// Call the `createPrivateTripForUserRef()` function to get a reference to the mutation.
const ref = createPrivateTripForUserRef(createPrivateTripForUserVars);
// Variables can be defined inline as well.
const ref = createPrivateTripForUserRef({ name: ..., destination: ..., startDate: ..., endDate: ..., description: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = createPrivateTripForUserRef(dataConnect, createPrivateTripForUserVars);
